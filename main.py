...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for id_produto, qtd in carrinho.items():
            ...
            texto += f" {qtd} × {produto['nome']} = ¥{(subtotal):,}".replace(",", ".") + f" | R$ {subtotal_real:.2f}\n"

            botoes.append([
                InlineKeyboardButton("+1", callback_data=f"mais:{id_produto}"),
                InlineKeyboardButton("-1", callback_data=f"menos:{id_produto}"),
                InlineKeyboardButton("❌ Cancelar item", callback_data=f"cancelar:{id_produto}")
            ])
        ...
        texto += f"\nTaxa Pix (0.99%): ¥{int(total_pix):,}".replace(",", ".") + f" | R$ {total_pix * VALOR_IENE_REAL:.2f}"
...
async def receber_preco(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"📸 Agora envie a foto do produto.\n"
        f" Preço: ¥{(preco):,}".replace(",", ".") + f" | R$ {valor_real:.2f}"
    )
    return FOTO
...
async def receber_foto(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        caption=(
            f"*{nome}*\n\n"
            f"_{descricao}_\n\n"
            f"🇯🇵 {(preco):,}".replace(",", ".") + f" ienes | 🇧🇷R$ {preco_real:.2f}"
        ),
        parse_mode="Markdown",
        reply_markup=botao
    )
...
async def botao_comprar(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        carrinhos[user_id] = carrinho
        await query.message.reply_text("✅ Produto adicionado ao carrinho. Use /carrinho para ver.")
...
async def ver_carrinho(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not carrinho:
        ...
        if update.message:
            await update.message.reply_text("\U0001F6D2 Seu carrinho está vazio.")
        # Se for via botão
        elif update.callback_query:
            await update.callback_query.edit_message_text("\U0001F6D2 Seu carrinho está vazio.")
        return
    ...
    for id_produto, qtd in carrinho.items():
        ...
        total_iene += subtotal
        subtotal_real = subtotal * VALOR_IENE_REAL
        texto += f" {qtd} × {prod['nome']} = ¥{subtotal:,}".replace(",", ".") + f" | R$ {subtotal_real:.2f}\n"
    ...
    botoes.append([InlineKeyboardButton("\u2705 Confirmar", callback_data="confirmar")])
    markup = InlineKeyboardMarkup(botoes)

    # Se for via /carrinho, envia nova mensagem. Se for via botão, edita a existente.
    if update.message:
        await update.message.reply_text(texto, parse_mode="Markdown", reply_markup=markup)
    elif update.callback_query:
        try:
            await update.callback_query.edit_message_text(texto, parse_mode="Markdown", reply_markup=markup)
...
async def carrinho_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Atualiza o carrinho com os novos valores
...
async def adicionar_ao_carrinho(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_caption(
        caption=f"✅ *{produto['nome']}* adicionado ao carrinho!\nUse /carrinho para ver seus itens.",
        parse_mode="Markdown"
    )
...
async def receber_comprovante(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("✅ Pedido enviado com sucesso! Obrigada pela compra.")
...
carrinho_conversa = ConversationHandler(
    entry_points=[
        CallbackQueryHandler(carrinho_callback, pattern="^(mais:|menos:|cancelar:).+|^confirmar$")
    ],
        states={
...
async def main():
    ...
    app.add_handler(CallbackQueryHandler(adicionar_ao_carrinho, pattern="^add_"))
